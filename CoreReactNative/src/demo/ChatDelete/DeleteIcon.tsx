...
export default function DeleteIcon({onPress, color}) {
  ...
  const animatedProps = useAnimatedProps(() => {
    ...
    const d = interpolatePath(progress.value, [1, 0], [p1, p2], [p1, p2, p2, p1], {
  ...
  const rotateStyle = useAnimatedStyle(() => {
    ...
    return {
      //   width: 100,
      transform: [{translateY: 10}, {translateX: 25}, {rotate: `${deg}deg`}, {translateY: -10}, {translateX: -25}],
    }
  })

  const handlePress = () => {
    rotate.value = withTiming(rotate.value ? 0 : 1, {duration: 300}, isFinished => {
      if (isFinished) {
        // insert animation 1
        ...
        <View style={{transform: [{scale: 0.8}]}}>
          <Svg width="35" height={'65'}>
            <G translateY={15}>
              ...
              {/* </Animated.View> */}
              {/* </AnimatedGroup> */}
              <AnimatedPath animatedProps={animatedProps} translateY={12} fill={color ? color : 'white'} />
            </G>
          </Svg>
        </View>
      </View>
