...
const Swipable = props => {
  ...
  useEffect(() => {
    if (props.selectedIndex === undefined || props.selectedIndex >= props.index) return
    staggersTranslate()
  ...
  const scaleStyle = useAnimatedStyle(() => {
    const scalee = interpolate(transform_origin_rotate_deg.value, [0, 90], [1, 0], CLAMP)
  ...
  return (
    <Animated.View style={[translateTopStyle, styles.container]}>
      <PanGestureHandler failOffsetY={[-5, 5]} activeOffsetX={[-5, 5]} onGestureEvent={gestureHandler}>
        <Animated.View>
          <Animated.Text style={[styles.deleteMsg, animatedTextStyle]}>Your message has been deleted</Animated.Text>
          ...
          {toggle && (
            <Animated.View style={[rotateStyle, {position: 'relative'}]}>
              <Animated.View style={animatedStyle}>
                <Animated.View style={[scaleStyle]}>{props.children}</Animated.View>
              </Animated.View>
            </Animated.View>
          )}
        </Animated.View>
