...
const SimplePopover = ({position, targetLayout, onPress}: SimplePopoverProps) => {
  ...
  const popoverPosition = useMemo(
    () => ({
      ...
      top:
        position === 'bottom' ? targetLayout.y + targetLayout.height + SPACE : targetLayout.y - layout.height - SPACE,
      left: targetLayout.x + targetLayout.width / 2 - layout.width / 2,
    }),
    [position, layout, targetLayout],
  )

  const popoverContainerStyle = useMemo(() => [styles.popoverContainer, popoverPosition], [popoverPosition])
