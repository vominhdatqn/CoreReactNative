...
const PortalComponent = ({name: _providedName, hostName, handleOnMount, handleOnUnMount, children}: PortalProps) => {
  // hooks
  const {addPortal, removePortal, updatePortal} = usePortal(hostName)
  const name = useMemo(() => _providedName || randomUniqueId(), [_providedName])
